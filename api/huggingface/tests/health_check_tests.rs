...
async fn test_ping_strategy_healthy_endpoint() 
{
  ...
  endpoint : "https://httpbin.org".to_string( ),
...
{
  let config = HealthCheckConfig {
  endpoint : "https://httpbin.org/status/404".to_string( ),
  strategy : HealthCheckStrategy::LightweightApi,
  check_interval : Duration::from_secs( 30 ),
  timeout : Duration::from_secs( 5 ),
...
async fn test_full_endpoint_strategy_accepts_client_errors() 
{
  ...
  let config = HealthCheckConfig {
  endpoint : "https://httpbin.org/status/400".to_string( ),
  strategy : HealthCheckStrategy::FullEndpoint,
  check_interval : Duration::from_secs( 30 ),
  timeout : Duration::from_secs( 5 ),
  unhealthy_threshold : 3,
...
{
  let config = HealthCheckConfig {
  endpoint : "https://httpbin.org/delay/10".to_string( ),  // 10 second delay
  strategy : HealthCheckStrategy::LightweightApi,
  check_interval : Duration::from_secs( 30 ),
...
{
  let config = HealthCheckConfig {
  endpoint : "http://httpbin.org".to_string( ),  // HTTP, will redirect to HTTPS
  strategy : HealthCheckStrategy::Ping,
  check_interval : Duration::from_secs( 30 ),
  timeout : Duration::from_secs( 5 ),
  unhealthy_threshold : 3,
