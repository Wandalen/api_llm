...
mod private
{
  ...
  async fn test_model_discovery()
  {
    ...
    assert!( result.is_ok(), "Should be able to query available models" );
  ...
  async fn test_invalid_model_name_error()
  {
    ...
    // Intentionally use invalid model name (without tag)
    let request = ChatRequest
    {
      model : "llama3.2".to_string(), // Missing size tag - should fail
      messages : vec!
      [
        ChatMessage
        {
  ...
  async fn test_chat_with_valid_model()
  {
    ...
    let request = ChatRequest
    {
      model : "llama3.2:3b".to_string(), // Proper format with tag
      messages : vec!
      [
        ChatMessage
        {
